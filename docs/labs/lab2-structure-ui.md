...
| 위치 | 무엇이 있나 | 언제 쓰나 |
|------|------------|----------|
| **Summary** | job 그래프, 소요 시간, 아티팩트 목록 | 전체 흐름 한눈에 확인 |
| **각 step 로그** | step별 출력, 접기/펼치기 | 오류 원인 추적 |
| **Workflow file** 탭 | 그 실행에 실제로 쓰인 워크플로 원본 | "분명 고쳤는데 옛날대로 돈다" 싶을 때 |
| 우측 **…** → **Disable workflow** | 워크플로 일시 정지 | 긴급하게 자동 실행을 막아야 할 때 |
