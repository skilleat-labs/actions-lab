...
## 목표
**호스팅 러너는 매번 새 VM** 이라는 것(self-hosted 는 다릅니다 — Lab 6)과, **job은 서로 격리**된다는 것을 눈으로 확인합니다.
...
> Actions는 (호스팅 러너가 매번 새 VM 이라) **직접 적어야** 소스가 생깁니다. (이 랩의 1-A가 그걸 눈으로 보여줍니다)
...
호스팅 러너는 매번 새 VM 이라, `actions/checkout`을 **명시**하지 않으면 소스가 없습니다. self-hosted 러너는 머신이 남아 있어 지난 실행의 소스가 보일 수 있고, checkout 이 `clean` 으로 정리합니다(Lab 6). (Jenkins 아시는 분: 자동이던 SCM 체크아웃이 여기선 명시)
