...
| 교시 | 질문 | 우리 답 |
|---|---|---|
| 1 | 우리 러너를 어디에 둘까 — 기존 빌드 VM 인가, Kubernetes(ARC) 인가? | |
| 2 | YAML 에 익숙해지는 기간을 누가 언제 쓸까 — 파일럿 팀 한 곳부터? | |
| 3 | 승인은 누가 하고, 산출물은 어디까지 GitHub, 어디부터 기존 저장소인가? | |
...
GHES 에 self-hosted 러너를 Kubernetes(ARC)로 운영 중인 팀의 후기입니다.

| 치른 비용 | 얻은 것 |
|---|---|
| 러닝커브가 기존 CI 도입 때보다 높았다. 워크플로 문법에 익숙해지는 기간이 필요 | 코드(YAML)로 관리되니 이력 관리와 작성이 간편, 익숙해지면 훨씬 빠르다 |
...
!!! quote "요약"
    전환에는 러닝커브와 시행착오가 있고, 그 시간은 투자로 봐야 한다. 견디면 장점이 많다.

    세 번째 줄(마켓플레이스 우회)이 Lab 4 의 허용 목록, Lab 6 의 actions-sync 로 다루는 바로 그 지점입니다.
    미리 절차를 만들어 두면 우회가 아니라 운영이 됩니다.
...
[← Lab 6 · 온프레미스 러너와 마이그레이션](lab6-runners-migration.md)  ·  [🏠 랩 목록](../)
