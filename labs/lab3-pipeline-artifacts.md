...
(Jenkins 아시는 분: `input` 은 파이프라인이 agent를 잡고 실행 중인 채로 멈추지만, 여기는 시작 전에 멈춥니다 — 통제가 더 강함)
...
### 도전 1 ★★ 매트릭스에 축 하나 더
**요구사항**: `build_type(Debug, Release)` 에 **`target(MCU, AP)`** 축을 추가해 4조합을 만들되, **`Debug × AP` 조합은 제외**합니다. 아티팩트 이름이 겹치면 안 됩니다.
**완료 조건**: build job이 **3개**로 갈라지고, 아티팩트 3개의 이름에 두 축이 모두 들어감. (`make`가 target을 모르니 `echo "target=${{ matrix.target }}"` 로 대신합니다)
**힌트**: `strategy.matrix` 의 `exclude` (반대로 `include`도 있음). 실무에서 ECU × 빌드타입 × 컴파일러 조합이 이 형태입니다.

### 도전 2 ★★ 테스트가 실패해도 로그는 남기기
**요구사항**: `make test` 출력을 `test.log` 파일에 저장하고, 테스트가 **실패해도** 그 파일이 아티팩트로 올라가게 합니다. (실패는 Lab 2-B처럼 `tests/test_logic.c`의 CHECK를 하나 틀리게)
**완료 조건**: job은 빨간불인데 Summary 하단에 `test-log` 아티팩트가 있음. 내려받아 열면 실패한 CHECK가 적혀 있음.
**힌트**: `tee`, 그리고 step의 `if:` 에 쓰는 상태 함수(`always()`, `failure()`). 어느 쪽이 "성공/실패 상관없이"인지 문서에서. Jenkins `post { always { archiveArtifacts } }` 자리입니다.

### 도전 3 ★ 빌드 시간 제한
**요구사항**: build job이 **3분**을 넘기면 자동으로 취소되게 합니다. 확인을 위해 step 하나에 `sleep 200` 을 넣어 실제로 끊기는지 봅니다.
**완료 조건**: 3분쯤에 job이 취소(빨간 X)되고 로그에 "timed out" 류 메시지.
**힌트**: job 레벨 키 하나입니다. 기본값은 360분 — 임베디드 빌드가 걸려서 6시간 동안 러너를 잡고 있는 사고를 막는 키입니다.

### 도전 4 ★★★ 릴리스 노트 자동 생성
**요구사항**: 3-E의 Release에 **본문(노트)** 을 넣습니다. (1) 이전 릴리스 이후의 커밋 목록이 자동으로 들어가고 (2) 첫 줄에 빌드 타입과 커밋 SHA 앞 7자리가 적혀 있게.
**완료 조건**: Releases 페이지에서 노트가 채워진 릴리스가 보임.
**힌트**: `gh release create` 의 옵션 중 노트를 자동 생성하는 것과 직접 넣는 것이 있습니다. SHA 7자리는 `${{ github.sha }}` 를 셸에서 자르면 됩니다 (Lab 3의 "실무 도전"과 같은 기법).
