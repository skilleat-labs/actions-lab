...
<details>
<summary>💡 답 보기 (먼저 스스로 생각한 뒤 펼치세요)</summary>

- 한 번의 push가 **여러 워크플로의 `on:` 조건에 동시에 맞으면**, 그것들이 다 (병렬로) 실행됩니다.
- 이번 commit 하나에 `src/` 파일과 `pipeline.yml`이 함께 담겼습니다.

| 워크플로 | `on:` 조건 | 이번 push에 맞았나 |
|---|---|---|
| Lab3 파이프라인 | `push` + `branches: main` | ✅ main에 push → 실행 |
| Lab2 CI | `push` + `paths: src/**` | ✅ `src/`가 바뀜 → 실행 |
| Lab1 첫 워크플로 | `workflow_dispatch` (수동만) | ❌ push로는 안 돎 |

- 즉 **파일이 여러 개**라서가 아니라, **여러 워크플로의 트리거가 이번 push에 동시에 맞아서**입니다.
- "왜 이게 돌았지 / 왜 안 돌지?" 는 항상 그 워크플로의 **`on:` 을 보면** 답이 나옵니다.
