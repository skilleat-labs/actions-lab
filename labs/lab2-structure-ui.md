...
**요구사항**: `*.md` 파일과 `docs/` 폴더 변경은 CI를 **건너뛰고**, `src/` 변경은 **돌게** 합니다.
**완료 조건**: README만 고쳐 push → Actions 탭에 아무것도 안 생김. `src/logic.c` 한 줄 고쳐 push → 실행됨.
**힌트**: `paths:` 의 반대 키가 있습니다. 단, 두 키를 **같은 트리거에 같이 쓸 수 없다**는 제약이 문서에 있습니다 — 그럼 어떻게 써야 할까요?
...
**완료 조건**: Run workflow 창에 체크박스가 보이고, 켰을 때만 해당 step이 초록, 껐을 때는 회색.
**힌트**: 입력 `type:` 에 `choice` 말고 어떤 값들이 있는지, step의 `if:` 에서 `inputs.verbose` 를 어떻게 참조하는지.
