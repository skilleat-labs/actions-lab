...
|---|---|---|
| Jenkins/CloudBees 빌드 VM | self-hosted 러너 (기존 VM 재활용) | |
...
| Artifactory 업로드 | (그대로 유지 — CI만 교체) | |
| PLM 전송 | 마지막 job의 API 호출 | |
| Bitbucket (소스) | GHES로 이관 (**별도 과제**) | |
...
> 원칙: **CI 전환과 Bitbucket 이관을 동시에 하지 않는다.**
...
### 지금 바로
- [ ] 현재 Jenkins/CloudBees 플러그인 목록 뽑기
- [ ] 파일럿 레포 하나 선정
- [ ] GHES 사양을 인프라팀에 전달

### 더 확인
- [ ] 사내 IdP/PLM의 OIDC(JWT) 지원 여부
