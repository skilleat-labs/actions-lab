...
## 목표
**호스팅 러너는 매번 새 VM** 이라는 것(self-hosted 는 다름, Lab 6)과, **job은 서로 격리**된다는 것을 눈으로 확인합니다.
...
호스팅 러너는 매번 새 VM 이라, `actions/checkout`을 **명시**하지 않으면 소스가 없습니다. (self-hosted 는 지난 실행의 소스가 남아 있을 수 있어 checkout 이 `clean` 으로 정리합니다 — Lab 6) (Jenkins 아시는 분: 자동이던 SCM 체크아웃이 여기선 명시)
